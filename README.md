# my:code

이미지를 올리면 웹에서 바로 쓸 수 있는 코드로 바꾸고, 내가 만든 코드를 날짜별로 차곡차곡 보관하는 초간편 이미지 코드 보관함.

> 이미지만 올리세요. 코드는 my:code가 알아서.

## 핵심 사용 흐름

```text
이미지 올리기
   ↓
자동 변환
   ↓
코드 생성
   ↓
MY CODE 자동 보관
   ↓
필요할 때 프로젝트로 정리
```

사용자에게 저장 위치나 프로젝트를 먼저 묻지 않습니다. **일단 올리면 자동으로 보관되고, 정리는 나중에 선택적으로** 하는 것이 my:code의 핵심 UX입니다.

## 현재 구현

- 이미지 드래그 앤 드롭 / 다중 업로드
- JPG/PNG/WEBP → WebP 자동 변환
- GIF/SVG 원본 유지
- 업로드 즉시 자동 저장 — 별도 저장 버튼 없음
- `MY CODE` 날짜별 자동 정렬
- 파일명 / 프로젝트 검색
- URL / HTML / CSS 코드 원클릭 복사
- 다중 선택 → 프로젝트 지정
- 프로젝트 폴더 생성
- 친근한 반응형 모바일 UI
- 서버가 연결되지 않아도 테스트 가능한 IndexedDB 로컬 저장
- Cloudflare Pages Functions + R2 업로드 엔드포인트 골격

## 현재 저장 방식

현재 프론트엔드만 실행하는 경우 변환 이미지는 브라우저 IndexedDB에 저장됩니다. 이 모드는 UI/UX 테스트용입니다.

- 같은 브라우저에서는 새로고침/재방문 후에도 이미지가 남습니다.
- CDN 서버가 연결되기 전에는 복사되는 코드는 이미지 자체가 포함된 Data URL입니다.
- 기기 간 동기화와 외부 CDN 영구 URL은 계정/서버 연결 후 활성화합니다.

## 최종 운영 구조

```text
사용자
  ↓
my:code
  ├─ Firebase Authentication : 간단 회원가입 / 로그인
  ├─ Firestore               : MY CODE / 프로젝트 / 메타데이터
  └─ Cloudflare R2           : 실제 이미지 원본·변환본
           ↓
      전용 CDN 주소
```

사용자가 이미지를 업로드하면:

1. 브라우저에서 WebP 최적화
2. R2에 이미지 저장
3. 고유 CDN URL 발급
4. URL / HTML / CSS 코드 자동 생성
5. 사용자 `MY CODE`에 자동 기록
6. 필요할 때만 프로젝트 폴더로 분류

## 다음 구현 순서

1. Firebase 이메일/Google 간편 로그인
2. Firestore에 MY CODE / PROJECT 데이터 저장
3. R2 실제 버킷 및 binding 연결
4. my:code 전용 CDN 도메인 연결
5. 삭제 대신 휴지통 보관
6. 기존 URL 유지 이미지 교체 기능
7. FREE / PRO 사용량 제한 및 결제
8. 광고 영역

## UX 원칙

**올리고 → 복사하고 → 필요할 때 정리한다.**

기능이 많아져도 사용자가 처음 마주하는 행동은 항상 하나, **이미지 올리기**로 유지합니다.
